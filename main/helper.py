...
def generate_csv_file(report, csv_data):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(temp_file_path, "w", newline='') as csv_file:
            ...
            csv_writer.writerow(["store_id", "last_one_hour uptime", "last_one_hour downtime", "last_one_hour unit", "last_one_day uptime",
                                "last_one_day downtime", "last_one_day unit", "last_one_week uptime", "last_one_week downtime", "last_one_week unit"])
...
################### Handling some part as a function for generate the report ##############################
...
def get_last_one_hour_data(store, utc_time, current_day, current_time):
    ...
    last_one_hour_logs = store.status_logs.filter(
        timestamp__gte=utc_time - datetime.timedelta(hours=1)).order_by('timestamp')
    
    print(last_one_hour_logs)
    # Get the status of the last log in the last one hour
    if last_one_hour_logs:
        last_one_hour_log_status = last_one_hour_logs.last().status
        if last_one_hour_log_status == StoreStatus.ACTIVE:
            last_one_hour_data["uptime"] = 60
        else:
            last_one_hour_data["downtime"] = 60
        print(last_one_hour_data)  # Print the data for debugging purposes
    return last_one_hour_data
...
def get_last_one_day_data(store, utc_time, current_day, current_time):
    ...
    is_store_open = store.timings.filter(
        day__gte=one_day_ago, day__lte=current_day, start_time__lte=current_time, end_time__gte=current_time).exists()
    if not is_store_open:
        return last_one_day_data
    # getting all the logs in last one day
    last_one_day_logs = store.status_logs.filter(
        timestamp__gte=utc_time - datetime.timedelta(days=1)).order_by('timestamp')
...
def get_last_one_week_data(store, utc_time, current_day, current_time):
    ...
    is_store_open = store.timings.filter(
        day__gte=one_week_ago, day__lte=current_day, start_time__lte=current_time, end_time__gte=current_time).exists()
    if not is_store_open:
        return last_one_week_data
    # getting all the logs in last one week
    last_one_week_logs = store.status_logs.filter(
        timestamp__gte=utc_time - datetime.timedelta(days=7)).order_by('timestamp')
